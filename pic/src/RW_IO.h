...
inline void readInputFile(struct parameters* param, int argc, char **argv){
...
inline void printParameters(struct parameters* param) {
    std::cout << std::endl;
    std::cout << "-------------------------" << std::endl;
...
inline void saveParameters(struct parameters* param) {
...
inline void Write_E_B_2D(int cycle, struct grid *grd, struct EMfield* field, struct parameters* param){
    ...
    std::ofstream my_fileEx(temp.c_str());
...
inline void Write_rho_J_2D(int cycle, struct grid *grd, struct interpDensSpecies* ids, struct interpDensNet* idn, struct parameters* param){
    ...
    double Jx_e = 0, Jy_e = 0, Jz_e = 0, Jx_i = 0, Jy_i = 0, Jz_i = 0, rho_e, rho_i;
    
    // Jx_e
    std::cout << "Opening file: " << temp << std::endl;
    std::ofstream my_fileJx_e(temp.c_str());
    ...
    for (int k=1; k < nzn-2; k++)
        for (int j=1; j < nyn-2; j++){
            for (int i=1; i < nxn-2; i++){
                rho_e = ids[0].rhon[i][j][k];
                my_filerho_e << rho_e << " ";
             }
            my_filerho_e << std::endl;
         }
    my_filerho_e.close();
    ...
    for (int k=1; k < nzn-2; k++)
        for (int j=1; j < nyn-2; j++){
            for (int i=1; i < nxn-2; i++){
                rho_i = ids[1].rhon[i][j][k];
                my_filerho_i << rho_i << " ";
             }
            my_filerho_i << std::endl;
         }
    my_filerho_i.close();
