...
class Kubernetes:
    ...
    def create_container(image, name, pull_policy):
        ...
        container = client.V1Container(
            ...
            #args=["sleep", "5"]
            command=["/usr/bin/python3", "kubv2/gromacs/launcher.py"],
        )
...
def scheduler():
    job_id = uuid.uuid4()
    _job_name = f"gmx-job-{job_id}"

    k8s = Kubernetes()
    # 1. Get the list of all active pods and check whether they start with "gromacs"
    ret = k8s.list_all_pods()
    for i in ret.items:
        print("%s\t%s\t%s" % (i.status.pod_ip, i.spec.nodeName, i.metadata.name))
